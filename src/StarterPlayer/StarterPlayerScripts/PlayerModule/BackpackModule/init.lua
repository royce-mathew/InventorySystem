...
local BACKGROUND_COLOR = Color3New(31/255, 31/255, 31/255)

local VR_FADE_TIME = 1
local VR_PANEL_RESOLUTION = 100

local SLOT_DRAGGABLE_COLOR = Color3New(0.176471, 0.176471, 0.176471)
local SLOT_DRAGGABLE_TRANSPARENCY = 0.4
local SLOT_EQUIP_COLOR = Color3New(1, 0.772549, 0.309804)
local SLOT_EQUIP_THICKNESS = 0 -- Relative
local SLOT_FADE_LOCKED = 0.6 -- Locked means undraggable
local SLOT_BORDER_COLOR = Color3New(0.411765, 0.619608, 1) -- Appears when dragging
...
local ARROW_HOTKEY = {Enum.KeyCode.Tab, Enum.KeyCode.DPadUp} --TODO: Hookup '~' too?
...
local SEARCH_BACKGROUND_COLOR = Color3New(0, 0, 0)
...
BackpackScript.InventoryIcon = InventoryIcon
...
InventoryIcon:disableStateOverlay(true)
InventoryIcon:setName("Inventory")
InventoryIcon:setImageYScale(1.12)
InventoryIcon:setOrder(-5)
InventoryIcon:setLeft()
...
local function NewGui(className, objectName)
	...
	newGui.BackgroundColor3 = Color3New(0, 0, 0)
	newGui.BackgroundTransparency = 1
	newGui.BorderColor3 = Color3New(0, 0, 0)
	newGui.BorderSizePixel = 0
	newGui.Size = UDim2New(1, 0, 1, 0)
	if className:match('Text') then
		newGui.TextColor3 = Color3New(1, 1, 1)
		newGui.Text = ''
...
local function UpdateScrollingFrameCanvasSize()
	...
	ScrollingFrame.CanvasSize = UDim2New(0, 0, 0, canvasSizeY)
end
...
local function UpdateBackpackLayout()
	HotbarFrame.Size = UDim2New(0, ICON_BUFFER + (NumberOfHotbarSlots * (ICON_SIZE + ICON_BUFFER)), 0, ICON_BUFFER + ICON_SIZE + ICON_BUFFER)
	HotbarFrame.Position = UDim2New(0.5, -HotbarFrame.Size.X.Offset / 2, 1, -HotbarFrame.Size.Y.Offset)
	InventoryFrame.Size = UDim2New(0, HotbarFrame.Size.X.Offset, 0, (HotbarFrame.Size.Y.Offset * NumberOfInventoryRows) + INVENTORY_HEADER_SIZE + (IsVR and 2*INVENTORY_ARROWS_BUFFER_VR or 0))
	InventoryFrame.Position = UDim2New(0.5, -InventoryFrame.Size.X.Offset / 2, 1, HotbarFrame.Position.Y.Offset - InventoryFrame.Size.Y.Offset - 6)

	local uic = Instance.new("UICorner")
	uic.CornerRadius = UDim.new(0.015, 0)
	uic.Parent = InventoryFrame

	ScrollingFrame.Size = UDim2New(1, ScrollingFrame.ScrollBarThickness + 1, 1, -INVENTORY_HEADER_SIZE - (IsVR and 2*INVENTORY_ARROWS_BUFFER_VR or 0))
	ScrollingFrame.Position = UDim2New(0, 0, 0, INVENTORY_HEADER_SIZE + (IsVR and INVENTORY_ARROWS_BUFFER_VR or 0))
	AdjustHotbarFrames()
...
local function MakeSlot(parent, index)
	...
	function slot:Readjust(visualIndex, visualTotal) --NOTE: Only used for Hotbar slots
		...
		SlotFrame.Position = UDim2New(0, centered - (ICON_SIZE / 2) + (sizePlus * factor), 0, ICON_BUFFER)
	end
		...
		local function assignToolData()
			if FFlagCoreScriptTranslateGameText2 then
				...
				if ToolTip and tool:IsA('Tool') then --NOTE: HopperBin
					...
					ToolTip.Size = UDim2New(0, width, 0, TOOLTIP_HEIGHT)
					ToolTip.Position = UDim2New(0.5, -width / 2, 0, TOOLTIP_OFFSET)
				end
			else
				...
				local icon = tool.TextureId
				ToolIcon.Image = icon
				ToolIcon.AnchorPoint = Vector2New(0.5,0.5)
	...
	function slot:UpdateEquipView(unequippedOverride)
		if not unequippedOverride and IsEquipped(self.Tool) then -- Equipped
			...
			SlotFrame.UIStroke.Transparency = 0.45
			if not HighlightFrame then
				...
				for _, data in pairs(dataTable) do
					...
					edgeFrame.Size = UDim2New(data[1], 0, data[2], 0)
					edgeFrame.Position = UDim2New(data[3], 0, data[4], 0)
			...
			if HighlightFrame then
				SlotFrame.UIStroke.Transparency = 0.8
	...
	SlotFrame.Size = UDim2New(0, ICON_SIZE, 0, ICON_SIZE)
	...
	slotFrameStroke.Transparency = 0.8
	slotFrameStroke.Thickness = 2
	slotFrameStroke.Parent = SlotFrame
	slotFrameStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
	...
	do
		...
		SelectionObj.Size = UDim2New(1, 0, 1, 0)
	...
	ToolIcon.AnchorPoint = Vector2New(0.5, 0.5)
	ToolIcon.Size = UDim2New(1, 0, 1, 0)
	ToolIcon.Position = UDim2New(0.5, 0, 0.5, 0)
	ToolIcon.Parent = SlotFrame

	ToolName = NewGui('TextLabel', 'ToolName')
	ToolName.AnchorPoint = Vector2New(0.5, 0.5)
	ToolName.Size = UDim2New(0.9, 0, 1, 0)
	ToolName.Position = UDim2New(0.5, 0, 0.5, 0)
	...
	if index <= NumberOfHotbarSlots then -- Hotbar-Specific Slot Stuff
		...
		ToolTip.BackgroundColor3 = Color3New(0.4, 0.4, 0.4)
		...
		if index < 10 or index == NumberOfHotbarSlots then -- NOTE: Hardcoded on purpose!
			...
			--SlotNumber.Font = Enum.Font.JosefinSans
			--SlotNumber.Size = UDim2New(0.15, 0, 0.15, 0)
			--SlotNumber.AnchorPoint = Vector2New(0.5, 1)
			--SlotNumber.Position = UDim2New(0.5, 0, 0.9, 0)
			--SlotNumber.TextStrokeTransparency = 0.55
			--SlotNumber.TextWrapped = false
	...
	do -- Dragging Logic
		...
		SlotFrame.DragBegin:Connect(function(dragPoint)
			...
			if startParent == UIGridFrame then
				...
				local newPosition = UDim2New(0, SlotFrame.AbsolutePosition.X - InventoryFrame.AbsolutePosition.X, 0, SlotFrame.AbsolutePosition.Y - InventoryFrame.AbsolutePosition.Y)
		...
		SlotFrame.DragStopped:Connect(function(x, y)
			...
			elseif CheckBounds(HotbarFrame, x, y) then
				...
				for i = 1, NumberOfHotbarSlots do
					...
					local offset = GetOffset(otherSlot.Frame, Vector2New(x, y))
	...
	if index > NumberOfHotbarSlots then
		...
		if InventoryFrame.Visible and not ViewingSearchResults then
			...
			ScrollingFrame.CanvasPosition = Vector2New(0, math.max(0, offset))
		end
	end
...
local selectDirection = Vector2New(0,0)
...
local function getInputDirection(inputObject)
	...
	if inputObject.KeyCode == Enum.KeyCode.Thumbstick1 then
		...
		if magnitude > 0.98 then
			local normalizedVector = Vector2New(inputObject.Position.x / magnitude, -inputObject.Position.y / magnitude)
			selectDirection =  normalizedVector
		else
			selectDirection = Vector2New(0,0)
		end
	elseif inputObject.KeyCode == Enum.KeyCode.DPadLeft then
		selectDirection = Vector2New(selectDirection.x - 1 * buttonModifier, selectDirection.y)
	elseif inputObject.KeyCode == Enum.KeyCode.DPadRight then
		selectDirection = Vector2New(selectDirection.x + 1 * buttonModifier, selectDirection.y)
	elseif inputObject.KeyCode == Enum.KeyCode.DPadUp then
		selectDirection = Vector2New(selectDirection.x, selectDirection.y - 1 * buttonModifier)
...
local selectToolExperiment = function(actionName, inputState, inputObject)
	...
	if inputDirection == Vector2New(0,0) then
		return
	end
...
function changeSlot(slot)
	...
	if slot.Frame == GuiService.SelectedObject and swapInVr then
		...
		if currentlySelectedSlot then
			...
			slot.Frame:TweenSizeAndPosition(startSize + UDim2New(0, 10, 0, 10), startPosition - UDim2New(0, 5, 0, 5), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, .1, true, function() slot.Frame:TweenSizeAndPosition(startSize, startPosition, Enum.EasingDirection.In, Enum.EasingStyle.Quad, .1, true) end)
...
local function MakeVRRoundButton(name, image)
	...
	newButton.Size = UDim2New(0, 40, 0, 40)
	newButton.Image = "rbxasset://textures/ui/Keyboard/close_button_background.png";

	local buttonIcon = NewGui('ImageLabel', 'Icon')
	buttonIcon.Size = UDim2New(0.5,0,0.5,0);
	buttonIcon.Position = UDim2New(0.25,0,0.25,0);
	buttonIcon.Image = image;
	buttonIcon.Parent = newButton;

	local buttonSelectionObject = NewGui('ImageLabel', 'Selection')
	buttonSelectionObject.Size = UDim2New(0.9,0,0.9,0);
	buttonSelectionObject.Position = UDim2New(0.05,0,0.05,0);
...
local corner = script.UICorner:Clone()
...
LeftBumperButton.Size = UDim2New(0, 40, 0, 40)
LeftBumperButton.Position = UDim2New(0, -LeftBumperButton.Size.X.Offset, 0.5, -LeftBumperButton.Size.Y.Offset/2)

RightBumperButton = NewGui('ImageLabel', 'RightBumper')
RightBumperButton.Size = UDim2New(0, 40, 0, 40)
RightBumperButton.Position = UDim2New(1, 0, 0.5, -RightBumperButton.Size.Y.Offset/2)

-- Make the Inventory, which holds the ScrollingFrame, the header, and the search box
InventoryFrame = NewGui('Frame', 'Inventory')
InventoryFrame.BackgroundTransparency = 1 -- BACKGROUND_FADE
...
VRInventorySelector.Position = UDim2New(0, 0, 0, 0)
VRInventorySelector.Size = UDim2New(1, 0, 1, 0)
...
selectorImage.Size = UDim2New(1, 0, 1, 0)
...
ScrollingFrame.CanvasSize = UDim2New(0, 0, 0, 0)
...
UIGridFrame.Size = UDim2New(1, -(ICON_BUFFER*2), 1, 0)
UIGridFrame.Position = UDim2New(0, ICON_BUFFER, 0, 5)
UIGridFrame.Parent = ScrollingFrame

UIGridLayout = Instance.new("UIGridLayout")
UIGridLayout.SortOrder = Enum.SortOrder.LayoutOrder
UIGridLayout.CellSize = UDim2New(0, ICON_SIZE, 0, ICON_SIZE)
UIGridLayout.CellPadding = UDim2New(0, ICON_BUFFER, 0, ICON_BUFFER)
...
ScrollUpInventoryButton.Size = UDim2New(0, 34, 0, 34)
ScrollUpInventoryButton.Position = UDim2New(0.5, -ScrollUpInventoryButton.Size.X.Offset/2, 0, INVENTORY_HEADER_SIZE + 3)
ScrollUpInventoryButton.Icon.Position = ScrollUpInventoryButton.Icon.Position - UDim2New(0,0,0,2)
ScrollUpInventoryButton.MouseButton1Click:Connect(function()
	ScrollingFrame.CanvasPosition = Vector2New(
...
ScrollDownInventoryButton.Icon.Position = ScrollDownInventoryButton.Icon.Position - UDim2New(0,0,0,2)
ScrollDownInventoryButton.Size = UDim2New(0, 34, 0, 34)
ScrollDownInventoryButton.Position = UDim2New(0.5, -ScrollDownInventoryButton.Size.X.Offset/2, 1, -ScrollDownInventoryButton.Size.Y.Offset - 3)
...
local gamepadHintsFrame = Utility:Create'Frame'
{
	...
	Size = UDim2New(0, HotbarFrame.Size.X.Offset, 0, (IsTenFootInterface and 95 or 60)),
...
local function addGamepadHint(hintImage, hintImageLarge, hintText)
	...
	{
		Name = "HintImage",
		Size = (IsTenFootInterface and UDim2New(0,90,0,90) or UDim2New(0,60,0,60)),
		BackgroundTransparency = 1,
	...
	{
		Name = "HintText",
		Position = UDim2New(0, (IsTenFootInterface and 100 or 70), 0, 0),
		Size = UDim2New(1, -(IsTenFootInterface and 100 or 70), 1, 0),
...
local function resizeGamepadHintsFrame()
	gamepadHintsFrame.Size = UDim2New(HotbarFrame.Size.X.Scale, HotbarFrame.Size.X.Offset, 0, (IsTenFootInterface and 95 or 60))
	gamepadHintsFrame.Position = UDim2New(HotbarFrame.Position.X.Scale, HotbarFrame.Position.X.Offset, InventoryFrame.Position.Y.Scale, InventoryFrame.Position.Y.Offset - gamepadHintsFrame.Size.Y.Offset)
	...
	for i = 1, #gamepadHints do
		gamepadHints[i].Size = UDim2New(1, 0, 1, -5)
		gamepadHints[i].Position = UDim2New(0, 0, 0, 0)
	...
	for i = 1, #gamepadHints do
		gamepadHints[i].Position = (i == 1 and UDim2New(0, 0, 0, 0) or UDim2New(0, gamepadHints[i-1].Position.X.Offset + gamepadHints[i-1].Size.X.Offset + spaceBetweenElements, 0, 0))
		gamepadHints[i].Size = UDim2New(0, (gamepadHints[i].HintText.Position.X.Offset + gamepadHints[i].HintText.TextBounds.X), 1, -5)
	end
end
...
do -- Search stuff
	...
	searchFrame.Size = UDim2New(0, SEARCH_WIDTH - (SEARCH_BUFFER * 2), 0, INVENTORY_HEADER_SIZE - (SEARCH_BUFFER * 2))
	searchFrame.Position = UDim2New(1, -searchFrame.Size.X.Offset - SEARCH_BUFFER, 0, SEARCH_BUFFER)
	...
	searchBox.Size = UDim2New(0.78, 0, 0.85, 0)
	searchBox.AnchorPoint = Vector2New(0 ,0.5)
	searchBox.Position = UDim2New(0.05, SEARCH_TEXT_OFFSET_FROMLEFT, 0.5, 0)
	...
	xButton.Text = 'x'
	xButton.Font = Enum.Font.Sarpanch
	xButton.ZIndex = 10
	...
	xButton.AnchorPoint = Vector2New(1,0.5)
	xButton.Size = UDim2New(0.15, 0, 1, 0)
	xButton.Position = UDim2New(0.98, 0, 0.5, 0)
	...
	local function search()
		...
		ScrollingFrame.CanvasPosition = Vector2New(0, 0)
		UpdateScrollingFrameCanvasSize()
